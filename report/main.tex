...
\subsection{Preliminary Optimizations}
...
Instead of checking for collisions between every pair of balls, the board is divided into a grid of cells such that collisions may only happen within neighboring cells. Although this method introduces an additional phase for the population of the grid at every simulation step, it is linear in the number of balls and drastically reduces the multiplicative constant for the remaining quadratic part.

To be precise, for each cell, collisions are resolved for balls within the cell itself, then against the \textit{forward-neighboring} cells (i.e. the right, down-left, down, and down-right neighbors). In this way, collisions are never resolved twice.
...
Our solution involves a number of worker threads, handled manually in the Thread-Oriented version, and encapsulated by a \texttt{FixedThreadPool} executor in the Task-Oriented version. We opted for long-living worker threads, implementing Agenda parallelism, thus removing any thread creation overhead.
...
\item \texttt{AtomicReference}, used to manage the concurrent access to the game's mutable state (e.g. the player scores) and the kick velocity of player balls.
\item \texttt{AtomicList}, used to manage the concurrent access to the mutable collection of balls within the game.
\end{itemize}
These monitor classes are implemented following the standard semantics, except for \texttt{SynchCell}, which features an additional \texttt{end()} method, used to signal that no more values will be put into the cell. It wakes up all threads that happen to be blocked on the \texttt{get()} method, causing an empty \texttt{Optional} to be returned.
...
The collision phase now uses a spatial grid. Its production size (150 cells) made even the two-ball base run run out of memory, so for JPF the cell size was enlarged with a single constant (\texttt{maxRadius} set to $0.4$), reducing the grid to 12 cells. This loses no collisions, because the balls are far smaller than a cell. The value is also the largest one that still yields three grid rows: the collision phase distributes rows across workers in two passes (even rows, then odd rows), and with only two rows both passes map to the same worker, leaving the second worker idle. Three rows give each of the two workers a row in the even pass, so the concurrency of the collision phase is actually exercised.
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}lcrrr@{}}
        ...
        \texttt{PooolJpf} (base)           & no errors & 17:27 & 5\,248\,431 & 968 \\
        \texttt{HumanFallsImmediately}     & no errors & 04:30 & 1\,368\,357 & 968 \\
        \texttt{AllSmallBallsFallTogether} & no errors & 05:12 & 1\,559\,076 & 968 \\
        \texttt{BoundaryCollision}         & no errors & 16:05 & 5\,115\,565 & 968 \\
        \texttt{ActualCollision}           & no errors & 20:01 & 5\,964\,275 & 968 \\
        \texttt{HumanAndBotFallSameTick}   & no errors & 04:24 & 1\,403\,211 & 1\,936 \\
        \texttt{MoreWorkersThanBalls}      & no errors & 36:16 & 8\,889\,535 & 6\,038 \\
        \bottomrule
    \end{tabular}
    \caption{JPF verification results on \texttt{pooolThreadOrientedJpf} with the spatial-grid collision phase. Two workers and two ticks, three workers for \texttt{MoreWorkersThanBalls}, with \texttt{PreciseRaceDetector} as listener and \texttt{NotDeadlockedProperty} active by default. Times are wall clock.}
    \label{tab:jpf_results}
\end{table}

Every scenario completed without violations, including \texttt{MoreWorkersThanBalls}, which had been the only intractable case under the previous collision algorithm. With the earlier flat triangular-matrix resolution that scenario saturated the 2 GB heap of JPF after seven and a half hours and around 94 million new states, and a retry with fewer balls still failed to terminate in practical time. The spatial-grid version completes it in 36 minutes within 413 MB. The gain came entirely from the smaller grid: fewer cells mean fewer shared read-write locations per tick, which is what the verifier's state space counts. Its \texttt{end = 6\,038}, against 968 elsewhere, reflects the extra termination orderings introduced by the third worker.
...
The suite was run with the \texttt{PreciseRaceDetector} listener, which certifies the absence of data races, while deadlock detection is provided by the default \texttt{NotDeadlockedProperty} that is active regardless of the listener. A single run therefore covers both properties. This is the configuration in which the collision phase genuinely engages both workers: with the three-row grid the even pass schedules one row on each worker, so \texttt{resolveCollision}, the only place a race was ever found, is now exercised under concurrent reads and writes from a second worker rather than on a single one. No race was reported in any scenario, confirming that the getter fix holds across all interleavings of the two-worker collision phase.
