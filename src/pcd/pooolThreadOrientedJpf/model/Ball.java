...
public class Ball {
	...
	public void updateState(long dt, Board ctx) {
		...
		this.pos.map(new Function<P2d, P2d>() {
            @Override
            public P2d apply(P2d value) {
                return value.sum(Ball.this.vel.get().mul(dt_scaled));
            }
        });
