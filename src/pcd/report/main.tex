...
\item \texttt{SimulationWorker}, which performs an assigned portion of the simulation workload. In the Task-Oriented version it is replaced simply by \texttt{java.lang.Thread} and managed by the Executor framework. In both cases, multiple instances are expected to be active during the game's runtime.
...
After the race fix, JPF detected a deadlock: worker threads blocked in \texttt{SynchCell.get()} with the coordinator already \texttt{TERMINATED}. The cause is a race between the worker's check of \texttt{gameState.isGameOver()} at the top of its loop and the next call to \texttt{workBox.get()}. When a ball falls into a hole, the worker processing it sets \texttt{gameOver = true}. The coordinator then exits its main loop and stops enqueueing work. Any other worker that had passed the \texttt{isGameOver()} check just before now blocks in \texttt{wait()}, because the only producer for that queue is gone. The defect is structural: coordinator and worker consult the same flag to decide termination, without coordination.

The fix replaced the shared-flag shutdown with a channel-closure pattern. \texttt{SynchCell} gained a \texttt{synchronized end()} method, and \texttt{get()} now returns \texttt{Optional<T>}, yielding \texttt{Optional.empty()} once closed. The worker loop became a plain consumer that exits on the empty value and no longer reads \texttt{gameState}. The coordinator calls \texttt{end()} on every work box as its last action, removing the race window by construction. The same change was backported to the production \texttt{pooolThreadOriented}.
...
\begin{itemize}
    ...
    \item \textbf{\texttt{MoreWorkersThanBallsJpf}:} configures more workers than balls, leaving one worker idle in \texttt{workBox.get()} for the entire simulation.
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}lcrrr@{}}
        ...
        \texttt{PooolJpf} (base)           & no errors    & 26:16   & 6\,947\,640 & 968 \\
        \texttt{HumanFallsImmediately}     & no errors    & 05:17   & 1\,259\,485 & 968 \\
        \texttt{AllSmallBallsFallTogether} & no errors    & 05:16   & 1\,738\,773 & 968 \\
        \texttt{BoundaryCollision}         & no errors    & 17:07   & 5\,485\,810 & 968 \\
        \texttt{ActualCollision}           & no errors    & 16:13   & 5\,528\,612 & 968 \\
        \texttt{HumanAndBotFallSameTick}   & no errors    & 03:44   & 1\,297\,036 & 1\,936 \\
        \texttt{MoreWorkersThanBalls}      & JPF OOM      & 7:34:32 & 93\,949\,952 & 76 \\
        \bottomrule
    \end{tabular}
    \caption{JPF verification results on \texttt{pooolThreadOrientedJpf}. Two workers and two ticks, with \texttt{PreciseRaceDetector} as listener and \texttt{NotDeadlockedProperty} active by default. Times are wall clock.}
    \label{tab:jpf_results}
\end{table}

All scenarios completed without violations, except \texttt{MoreWorkersThanBallsJpf}. That run saturated the 2 GB heap of JPF after seven and a half hours and around 94 million new states. A second attempt with three workers and fewer balls also failed to terminate within practical time. The outcome is an empirical limit of the verifier on this workload. The state space scales with the number of worker threads, and above three workers it exceeds the resources of commodity hardware. The property tested by this scenario, the correct release of an idle worker by \texttt{end()}, is already covered by the other configurations, since every worker spends most of its time blocked in \texttt{workBox.get()} between phases.
...
A separate run of the base configuration with the \texttt{DeadlockAnalyzer} listener produced the same state count, end count, and maximum depth as the corresponding \texttt{PreciseRaceDetector} run. This confirms that the two listeners do not alter the explored state space. Deadlock detection is performed by the default \texttt{NotDeadlockedProperty}, no matter which listener is active. The listener only enriches the diagnostic output in case of violation. Repeating the entire scenario suite with \texttt{DeadlockAnalyzer} was therefore omitted as redundant.
